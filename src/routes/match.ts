...
router.post("/", async (req: Request, res: Response) => {
  ...
  const recommendation =
    match_score >= 70 ? "Strong match — you should apply for this role."
    : match_score >= 40 ? "Moderate match — consider upskilling in the missing areas."
    : "Low match — significant skill gaps. Focus on the missing skills first.";

  res.json({ match_score, matched_skills, missing_skills, recommendation });
});
